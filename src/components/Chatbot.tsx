...
export default function Chatbox() {
    ...
    const fetchAnalysisHistory = async () => {
        try {
            ...
            const data = await response.json();
            const history: AnalysisEntry[] = data.history || [];
    ...
    useEffect(() => {
        ...
        newSocket.on("sessionId", (id: string) => {
            console.log("📌 Session ID received:", id);
            setSessionId(id);
        });

        newSocket.on("aiMessage", (response: string | { response: string }) => {
            const aiText = typeof response === "string" ? response : response.response;
            setChatHistory(prev => [...prev, { sender: "AI", text: aiText }]);
        });
    ...
    const generateReport = async () => {
        ...
        try {
            const res = await fetch(`https://mentalhealthplatform.onrender.com/api/report/${sessionId}`, {
                method: "GET",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
            });
            ...
            const a = document.createElement("a");
            a.href = url;
            a.download = `chat_report_${sessionId}.txt`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setChatHistory(prev => [
                ...prev,
                { sender: "System", text: "Error uploading report." }
            ]);
        }
    };
    ...
    return (
        <div className="flex flex-col h-screen w-screen bg-gradient-to-br from-[#34e89e] to-[#0f3443] text-white p-6">
            ...
            <div className="flex items-center gap-4 p-4 bg-gray-900 rounded-lg">
                ...
                <button 
                    onClick={sendMessage} 
                    className="bg-blue-500 px-4 py-2 rounded-lg shadow-lg hover:bg-blue-600 transition-all"
                >
                    Send
                </button>
                <button 
                ...
                <label 
                    ...
                    className="bg-yellow-500 px-4 py-2 rounded-lg shadow-lg hover:bg-yellow-600 transition-all cursor-pointer"
                >
                    Upload Report
                </label>
                <button 
                    onClick={handleShowChart} 
                    className="bg-purple-500 px-4 py-2 rounded-lg shadow-lg hover:bg-purple-600 transition-all"
                >
                    View Progress Chart 📊
                </button>
                <button 
